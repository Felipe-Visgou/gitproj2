/***************************************************************************
 *  $MCI Modulo de implementacao: TAB Tabuleiro de Gamao
 *
 ...
 *	  2    fvc, tbm 	23/set/2015 termino do desenvolvimento
 *        1    fvc, tbm         21/set/2015 inicio desenvolvimento
...
TAB_tpCondRet TAB_CriarTabuleiro(TAB_tppTabuleiro * pTab,
    ...
    *pTab = (TAB_tppTabuleiro) malloc(sizeof (TAB_tpTabuleiro));
    ...
    // Pretas
    LIS_InserirElementoApos(vtCasa[23], vtPecaP[jp]);
    jp++;
    LIS_InserirElementoApos(vtCasa[23], vtPecaP[jp]);
    jp++;
    // Brancas
    LIS_InserirElementoApos(vtCasa[0], vtPecaB[jb]);
    jb++;
    LIS_InserirElementoApos(vtCasa[0], vtPecaB[jb]);
    jb++;
    for (i = 0; i < 5; i++) {
        // Pretas
        LIS_InserirElementoApos(vtCasa[5], vtPecaP[jp]);
        jp++;
        LIS_InserirElementoApos(vtCasa[12], vtPecaP[jp]);
        jp++;
        // Brancas
        LIS_InserirElementoApos(vtCasa[11], vtPecaB[jb]);
        jb++;
        LIS_InserirElementoApos(vtCasa[18], vtPecaB[jb]);
        jb++;
    }
    for (i = 0; i < 3; i++) {
        // Pretas
        LIS_InserirElementoApos(vtCasa[7], vtPecaP[jp]);
        jp++;
        // Brancas
        LIS_InserirElementoApos(vtCasa[16], vtPecaB[jb]);
        jb++;
    }
...
/***************************************************************************
 *
 *  Funcao: TAB  &Mover Pecas tabuleiro
 * 
 ***************************************************************************/
